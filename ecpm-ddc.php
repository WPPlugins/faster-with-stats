...
Text Domain: ecpm-ddc
*/

define('ECPM_DDC', 'ecpm-ddc');
define('ECPM_DDC_NAME', 'Faster with Stats');
define('ECPM_DDC_VERSION', '1.3.0');
define('ECPM_DDC_DB_VER', '1.2');
...
add_action('admin_menu', 'ecpm_ddc_create_menu_set', 11);
  
function ddc_is_pro(){
  if ( file_exists( plugin_dir_path(__FILE__) . '/ddc-pro.php' ) ) {
    return true;
  }
  return false;
}
...
if ( ddc_is_pro() ) {
  require_once( plugin_dir_path(__FILE__) . '/ddc-pro.php' );
} 
...
function ecpm_ddc_activate() {
  ...
  if ( $ecpm_ddc_installed != 'yes' ) {
    ...
    update_option( 'ecpm_ddc_db_ver', ECPM_DDC_DB_VER );            
  }
}
...
function ecpm_ddc_create_menu_set() {
  if ( is_plugin_active('easycpmods-toolbox/ecpm-toolbox.php') ) {
    ...
    if ($ecpm_etb_settings['group_settings'] == 'on') {
      add_submenu_page( 'ecpm-menu', ECPM_DDC_NAME, ECPM_DDC_NAME, 'manage_options', 'ecpm_ddc_settings_page', 'ecpm_ddc_settings_page_callback' );
      return;
    }
  }
  add_options_page(ECPM_DDC_NAME, ECPM_DDC_NAME,'manage_options', 'ecpm_ddc_settings_page','ecpm_ddc_settings_page_callback');;
}    
...
function ecpm_ddc_settings_page_callback() {
  ...
  ?>
      ...
      <?php
        echo "<p><i>Plugin version: <u>".ECPM_DDC_VERSION."</u>";
        echo "<br>Plugin language file: <u>ecpm-ddc-".get_locale().".mo</u></i></p>";
        ?>
      <div id='ddc-container-left' style='float: left; margin-right: 285px;'>
        <?php
        ...
        <?php require_once( plugin_dir_path(__FILE__) . '/'.str_replace('_', '-', $active_tab).'.php' );?>
      </div>
